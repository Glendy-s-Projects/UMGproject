    ...
    <AppLayout title="Segundo Semestre" activeTopicId="2">
      <div className="flex flex-col gap-2 min-h-screen bg-background text-foreground p-4">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="bg-surface-container-low border border-outline-variant p-4 rounded-xl shadow-sm mb-4">
            <select
              value={mode2}
              onChange={(e) => setMode2(e.target.value as Mode2)}
              className="p-2 border border-outline-variant bg-surface-container-lowest text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary rounded-md w-full transition-all"
            >
          ...
          {mode2 === "polar-to-components" && (
            <div className="flex flex-row gap-2 ">
              <div className="w-20 flex items-center justify-center flex-col">
                ...
                <input
                  className="w-full p-2 border border-outline-variant bg-surface-container-lowest text-on-surface focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary rounded-md mb-2 transition-all"
                  value={mag}
          ...
          {(mode2 === "sum-vectors" ||
            ...
            <div className="flex flex-col items-center justify-center mx-auto w-full">
              <div className="flex flex-row items-center justify-center gap-2 ">
                {vectors.map((_, i) => (
                  ...
                  >
                    ...
                    <BotonUtil
                      className="px-3 py-2 bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors shadow rounded-md"
                      onClick={() =>
              ...
              <BotonUtil
                className="px-3 py-2 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow rounded-md mb-2"
                onClick={() =>
          ...
          <div className="flex gap-2 mb-4">
            <BotonUtil
              label="Calcular"
              className="px-4 py-2 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow rounded-md cursor-pointer"
              onClick={handleCompute}
            />

            <BotonUtil
              label="Limpiar"
              className="px-4 py-2 border border-outline-variant bg-surface-container-lowest text-on-surface hover:bg-surface-container-low transition-colors shadow-sm rounded-md"
              onClick={() => {
                setOutput([]);
              }}
            />
        ...
        <div className="flex flex-col items-center  justify-center gap-4">
          <div className="mt-2 flex flex-col gap-2">
            ...
            {output.length === 0 ? (
              <div className="text-muted-foreground text-sm">No hay cálculos todavía.</div>
            ) : (
              output.map((element, i) => (
                <div key={i} className="bg-surface-container-lowest border border-outline-variant p-3 rounded-xl shadow-sm">
                  {element}
                </div>
              ))
            )}
          ...
          <div className="w-96">
            {chartDataAndOptions.datasets.length > 0 && (
              <div className="p-4 border border-outline-variant bg-surface-container-lowest rounded-xl shadow-sm">
