...
const Limites = () => {
  ...
  const simplificarIndeterminacion = (
    expr: string,
    punto: number,
  ): { simplificada: string; pasos: string[]; valor: number | null } => {
    const pasos: string[] = [];
    try {
      ...
      if (node.type === "OperatorNode" && (node as any).fn === "divide") {
        ...
        pasos.push(
          `\\text{Forma indeterminada } \\frac{0}{0} \\text{ detectada}`,
        );
        ...
        try {
          ...
          const valorSimplificado = evaluarFuncion(
            `(${numSimplified})/(${denSimplified})`,
            punto,
          );
          ...
          if (valorSimplificado !== null && isFinite(valorSimplificado)) {
            ...
            pasos.push(
              `\\text{Sustituyendo } x = ${punto}: ${valorSimplificado.toFixed(4)}`,
            );
            return {
              simplificada: `(${numSimplified})/(${denSimplified})`,
              pasos,
              valor: valorSimplificado,
            };
          }
        } catch (e) {
          // Si falla la simplificación, intentar L'Hôpital numéricamente
          const h = 0.0001;
          const derivadaNumerador =
            (evaluarFuncion(numerador, punto + h)! -
              evaluarFuncion(numerador, punto - h)!) /
            (2 * h);
          const derivadaDenominador =
            (evaluarFuncion(denominador, punto + h)! -
              evaluarFuncion(denominador, punto - h)!) /
            (2 * h);
          ...
          if (Math.abs(derivadaDenominador) > 0.0001) {
            ...
            pasos.push(
              `\\text{Aplicando regla de L'Hôpital (aproximación numérica)}`,
            );
            pasos.push(
              `\\lim_{x \\to ${punto}} \\frac{f'(x)}{g'(x)} \\approx ${resultado.toFixed(4)}`,
            );
  ...
  const obtenerExpresionPorPartes = (
    x: number,
    esExacto: boolean = false,
  ): string | null => {
    for (const parte of partes) {
      const cumpleCondicion = evaluarCondicion(
        parte.condicion,
        parte.valor || "0",
        x,
      );
  ...
  const calcularLimite = () => {
    ...
    try {
      ...
      const pasos: string[] = [];
      const expresionMostrar = esPorPartes
        ? generarLatexPorPartes()
        : `f(x) = ${convertirALatex(funcion)}`;
      ...
      if (esPorPartes) {
        ...
        pasos.push(
          `\\text{Para } x < ${punto}: f(x) = ${convertirALatex(expresionIzq || "")}`,
        );
        pasos.push(
          `\\text{Límite por la izquierda: } \\lim_{x \\to ${punto}^-} f(x) = ${limitePorIzquierda?.toFixed(4) || "indefinido"}`,
        );

        pasos.push(
          `\\text{Para } x > ${punto}: f(x) = ${convertirALatex(expresionDer || "")}`,
        );
        pasos.push(
          `\\text{Límite por la derecha: } \\lim_{x \\to ${punto}^+} f(x) = ${limitePorDerecha?.toFixed(4) || "indefinido"}`,
        );

        if (expresionPunto && valorDirecto !== null) {
          pasos.push(
            `\\text{Valor en el punto: } f(${punto}) = ${valorDirecto.toFixed(4)}`,
          );
        }
      } else {
        ...
        if (
          valorDirecto === null ||
          !isFinite(valorDirecto) ||
          isNaN(valorDirecto)
        ) {
          const resultadoSimplificacion = simplificarIndeterminacion(
            funcion,
            punto,
          );
        ...
        pasos.push(
          `\\text{Límite por la izquierda: } \\lim_{x \\to ${punto}^-} f(x) \\approx ${limitePorIzquierda?.toFixed(4) || "indefinido"}`,
        );
        pasos.push(
          `\\text{Límite por la derecha: } \\lim_{x \\to ${punto}^+} f(x) \\approx ${limitePorDerecha?.toFixed(4) || "indefinido"}`,
        );
      }
      ...
      if (
        ...
        if (Math.abs(limitePorIzquierda - limitePorDerecha) < 0.01) {
          ...
          limite = valorLimite.toFixed(4);
          pasos.push(
            `\\text{Como } \\lim_{x \\to ${punto}^-} f(x) = \\lim_{x \\to ${punto}^+} f(x), \\text{ el límite existe}`,
          );
          pasos.push(`\\therefore \\lim_{x \\to ${punto}} f(x) = ${limite}`);

          if (
            valorDirecto !== null &&
            Math.abs(valorDirecto - valorLimite) < 0.01
          ) {
            pasos.push(
              `\\text{Como } f(${punto}) = ${valorDirecto.toFixed(4)} = \\lim_{x \\to ${punto}} f(x), \\text{ continua en } x = ${punto}`,
            );
          } else if (valorDirecto !== null) {
            pasos.push(
              `\\text{Como } f(${punto}) = ${valorDirecto.toFixed(4)} \\neq \\lim_{x \\to ${punto}} f(x), \\text{ discontinuidad removible}`,
            );
          }
        } else {
          ...
          pasos.push(
            `\\text{Los límites laterales son diferentes, el límite no existe}`,
          );
          pasos.push(`\\text{Discontinuidad de salto en } x = ${punto}`);
        }
      } else {
        if (
          !isFinite(limitePorIzquierda || 0) ||
          !isFinite(limitePorDerecha || 0)
        ) {
          limite = "\\pm\\infty";
          esIndeterminado = true;
      ...
      setResultado({
        expresion: esPorPartes
          ? generarLatexPorPartes()
          : convertirALatex(funcion),
        punto: String(punto),
      ...
      <div className="flex flex-col min-h-screen p-4 max-w-7xl mx-auto">
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
          ...
          <ul className="list-disc list-inside space-y-1 text-sm">
            <li>
              Usa <code className="bg-white px-1 rounded">x</code> como variable
            </li>
            <li>
              Operaciones:{" "}
            ...
            <li>
              Raíz cuadrada:{" "}
              <code className="bg-white px-1 rounded">sqrt(x)</code>
            </li>
            <li>
              Funciones:{" "}
        ...
        <div className="mt-6 p-6 bg-white rounded-lg shadow-lg border">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          actualizarParte(index, "expresion", e.target.value)
                        }
                        placeholder="Ej: x^2"
          ...
          <div className="mt-4">
            <label className="block font-semibold mb-2">
              Punto donde evaluar el límite (x →):
            </label>
            <input
              type="text"
              value={puntoLimite}
            ...
            {datosGrafica.length > 0 && (
              <div className="p-6 bg-white rounded-lg shadow-lg border">
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                      ...
                      <ReferenceLine
                        ...
                        strokeDasharray="5 5"
                        label={{
                          value: `x = ${resultado.punto}`,
                          position: "top",
                        }}
                      />
                      {resultado.valorLimite !== null &&
                        isFinite(resultado.valorLimite) && (
                          <ReferenceLine
                ...
                <div className="mt-4 text-sm text-gray-600 text-center">
                  <p>
                    <span className="text-red-600 font-bold">Línea roja:</span>{" "}
                    Punto del límite
                    {resultado.valorLimite !== null &&
                      isFinite(resultado.valorLimite) && (
                        <>
                ...
                {resultado.esPorPartes ? (
                  <p className="text-sm">
                    ...
                    {resultado.esIndeterminado &&
                      " Se detectó una discontinuidad."}
                  </p>
                ) : resultado.esIndeterminado ? (
                  <p className="text-sm">
                    Forma indeterminada detectada. Se calcularon los límites
                    laterales.
                  </p>
                ) : (
                  <p className="text-sm">
